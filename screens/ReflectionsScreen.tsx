...
import MoodTracker from '../components/sections/Reflections/MoodTracker';
import Gratitude from '../components/sections/Reflections/Gratitude';
import JournalPrompt from '../components/sections/Reflections/JournalPrompt';
...
export default function ReflectionsScreen() {
    ...
    navigation.setOptions({
      headerTitle: () => (
        <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'flex-start', flex: 1 }}>
          <Text style={{ fontSize: 28, marginRight: 8 }} accessibilityLabel="Reflections book icon">📖</Text>
          <Text style={{ fontSize: 22, fontWeight: 'bold' }} accessibilityLabel="Reflections section header">Reflections</Text>
        </View>
      ),
    });
  }, [navigation]);
  ...
  // Toggle handler for horoscope entitlement (controls Reflections)
  const toggleHoroscope = () => {
    setEntitlement({
      ...entitlement,
      horoscope: !entitlement.horoscope,
    });
  };

  // All sections locked unless horoscope entitlement is true
  const lockedStates = [
    !entitlement.horoscope,
    !entitlement.horoscope,
    !entitlement.horoscope,
    !entitlement.horoscope,
    !entitlement.horoscope,
  ...
  return (
    <View
      ...
      accessibilityLabel="Reflections main content"
    >
      <View style={{ alignItems: 'flex-end', marginRight: 16, marginBottom: 4 }}>
        <Button
          title={entitlement.horoscope ? 'Lock Reflections' : 'Unlock Reflections'}
          onPress={toggleHoroscope}
          
          accessibilityLabel={entitlement.horoscope ? 'Lock Reflections access' : 'Unlock Reflections access'}
        />
      </View>
      ...
      <ScrollView
        ...
        accessibilityLabel="Reflections sections scrollable area"
      >
